...
public class Change extends AbstractChange {
     ...
     */
    protected Change(Issue issue, Type type, int id, Calendar createdDate, User createdUser, Calendar modifiedDate,
                     User modifiedUser, Attribute attribute, String oldValue, String newValue) {
        super(issue, type, id, createdDate, createdUser, modifiedDate, modifiedUser, attribute, oldValue, newValue);
    }

    /*
     * Internal constructor.
     */
    protected Change(Issue issue, int id, Calendar createdDate, User createdUser, Calendar modifiedDate,
                     User modifiedUser, Attribute attribute, String oldValue, String newValue) {
        this(issue, Type.VALUE_CHANGED, id, createdDate, createdUser, modifiedDate, modifiedUser, attribute, oldValue,
                        newValue);
    }
     ...
     *         attachment response
     */
    static Change createFromResponse(Issue issue, List<String> response, Users users, IEnvironment environment) {
        if (environment.getVersion().startsWith("0.")) {
            ...
            return new Change(issue, Integer.parseInt(response.get(1)), date, user, date, user, attributeObject,
            ...
            Attribute attributeObject = environment.getTypes().getAttribute(attributeId);
            return new Change(issue, type, Integer.parseInt(response.get(1)), Util.parseTimestampInSeconds(response.get(5)),
